...
if ( ! function_exists( 'wp_cloud_storage_comment' ) ) :
/**
 * Template for comments and pingbacks.
 *
 * Used as a callback by wp_list_comments() for displaying the comments.
 */
function wp_cloud_storage_comment( $comment, $args, $depth ) {
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
		<div class="comment-body">
			<?php _e( 'Pingback:', 'wp_cloud_storage' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'wp_cloud_storage' ), '<span class="edit-link">', '</span>' ); ?>
		</div>
		...
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
			<footer class="comment-meta">
				<div class="comment-author vcard">
					...
					<?php printf( __( '%s <span class="says">says:</span>', 'wp_cloud_storage' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
				</div><!-- .comment-author -->

				<div class="comment-metadata">
					<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
						<time datetime="<?php comment_time( 'c' ); ?>">
							<?php printf( _x( '%1$s at %2$s', '1: date, 2: time', 'wp_cloud_storage' ), get_comment_date(), get_comment_time() ); ?>
						</time>
					</a>
					<?php edit_comment_link( __( 'Edit', 'wp_cloud_storage' ), '<span class="edit-link">', '</span>' ); ?>
				</div><!-- .comment-metadata -->

				<?php if ( '0' == $comment->comment_approved ) : ?>
				<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'wp_cloud_storage' ); ?></p>
				<?php endif; ?>
			</footer><!-- .comment-meta -->
...
endif; // ends check for wp_cloud_storage_comment()

if ( ! function_exists( 'wp_cloud_storage_the_attached_image' ) ) :
/**
 * Prints the attached image with a link to the next attached image.
 */
function wp_cloud_storage_the_attached_image() {
	$post                = get_post();
	$attachment_size     = apply_filters( 'wp_cloud_storage_attachment_size', array( 1200, 1200 ) );
